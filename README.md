...
void employ() {
    while (true) {
        ...
        for (int i = 1; i < tables.size(); i++) {
            ...
            string status = (tables[i].count > 0) ? "NOT AVAILABLE" : "AVAILABLE";

            cout << left << setw(8) << i
                << setw(20) << status
                << setw(12) << tables[i].count
                << right << setw(10) << tables[i].total
                << endl;
        }
        ...
        if (tables[check].count > 0) {
            ...
            for (int i = 0; i < tables[check].count; i++) {
                cout << left << setw(5) << i + 1
                    ...
                    << setw(12) << tables[check].Price[i] * tables[check].Amount[i]
                    << endl;
            }
